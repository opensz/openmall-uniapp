...
export default {
	...
	async onLoad(ops) {
		...
		if (id) {
			...
			let data = await Api.apiCall('get', Api.goods.schoolDetail, params);
			this.logining = false;
	...
	methods: {
		...
		toFavorite(item) {
			if (!this.hasLogin) {
				...
				let params = { objId: item.id, type: 6, memberId: this.userInfo.userInfo.id, name: item.name, meno1: item.pic, meno2: item.price, meno3: item.sale };
